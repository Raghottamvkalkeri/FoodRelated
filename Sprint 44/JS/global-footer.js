...
var OrderguideSearchProducts = new Vue({
  ...
  methods: {
    ...
    getWishlistName(result) {
      const wishlistNames = result.data.map((item) => item.title);
      this.orderguideProducts = wishlistNames;
    },
    ...
    makeAjaxRequest(url, jsonRequest, showLoader = true, toggleVisibility = true , source) {
      ...
      $.ajax({
        ...
        complete() {
          ...
          if (showLoader) {
            document.querySelector('.searchproductlist').scrollTo({ top: 0 });
          }
