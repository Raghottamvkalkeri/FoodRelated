        ...
        if (
          ...
          $(data)
            ...
            .each(function () {
              ...
              $(".error-message").html(
                "There is error in " + $(this).find("label").text()
              );
            });
          ...
          jQuery("#shippingContent").html(
            `<div style="position: absolute;width: 100%;height: 100%;background: transparent;z-index: 999;top: 0px;bottom: 0px;" class="shippingContentloaderbox" hidden="">
...
$("body").on("submit", "#opay", function (e) {
  ...
  if ($('input[name="ShippingMethod"]').is(":checked") == false) {
    $(".new-error-message.error-message")
      .show()
      .html("Please Select Shipping Method");
  } else {
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        if (
          ...
          if($(data).find(".error-message").html().includes('Invalid shipping method.') === true ) {
        ...
        inline: "nearest",
          ...
          if (
            document.querySelector("input[name=curbsy_datanew]") !== null &&
            document.querySelector("input[name=curbsy_datenew]") !== null
          ) {
            $("input[name=curbsy_data]").val(
              document.querySelector("input[name=curbsy_datanew]").value
            );
            $("input[name=curbsy_date]").val(
          ...
          if ($("#getfromwhichsection").val() == "shipping") {
          } else {
            $(".savedPaymentMethod").each(function () {
          ...
          if ($("#redeemablebalance:checked").val() == "points:points") {
            ...
            $(".newcard").attr(
              "onclick",
              '$(".newcardradio").click();$("#paymentmodal").modal({backdrop: "static", keyboard: false},"show")'
            );
          }
          ...
          $(".ispaymentAccordianHeaderVisible").removeClass(
            "ispaymentAccordianHeaderVisible"
          );
          $(".isdeliveryAccordianBodyvisible").removeClass(
            "isdeliveryAccordianBodyvisible"
          );
        ...
        $("body").on("click", ".continueOrder", function () {
          ...
          $("#applyCouponForm")
            ...
            .val(sessionStorage.getItem("SelectedPaymentMethod"));
          $(".orderSummaryMainContainer").show();

          $(".savedPaymentMethobd").each(function () {
            if ($(this).is(":checked")) {
              $("#cardDetail").text(
                "Card ends in " + $(this).attr("data-prompt")
              );
          ...
          $("#edit,#back").show();
          $("#reivewSelectedDeliveryMethod").html(
            $("#selectedDelivery").html()
          );
          ...
          $.ajax({
            ...
            data: {
              ...
              delivery_date: jQuery("#paymentmethodForm")
                .find('input[name="delivery_date"]')
                .val(),
            },
          }).done(function (msg) {
            console.log("done");
  ...
  if ($(this).is(":checked") == true) {
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        if (
          $("#savedCardDetails").find('input[name="AmountType"]').val() ==
          "total"
        ) {
        ...
        $(".newcard").attr(
          "onclick",
          '$("#AdditionalPaymentMethodUsenewCard").click()'
        );
        $(".newcard").show();
      },
    });
...
var onclick_submit = function (event) {
  ...
  setTimeout(function () {
    ...
    if (checkout_store_data == 1) {
      ...
      if (
        TDate(showfromattedDate) == false ||
        CustomerCutoffTime() == false ||
        checkisRewardPaymentisavailable() == true
      ) {
      } else {
        var payment_terms = "";
        if ($(".savedPaymentMethod:checked").val() == "paypalcp:paypal") {
          payment_terms = jQuery("body")
            .find("#opay_form3")
            .find(".selectedPaymentMethods")
            .val()
            .split(":");
        ...
        if (
          payment_terms[0] == "mivapay" ||
          payment_terms[0] == "paymentcard"
        ) {
          ...
          console.log(
            "formated date" + getformatted + "current date" + getcurrentdate
          );
...
const getOrderSummary = () => {
  ...
  let url =
    "/ajax.html?CustomerAction=getbasketcharges&displayType=raw&redeemed=" +
    isredeemed;
  $.get(url, (response) => {
...
var getCreditfeeSummary = () => {
  ...
  $.get(url, (response) => {
    ...
    if (typeofUser == "1") {
      ...
      if (sessionStorage.getItem("threebieSavings") == null) {
        ...
        $(".showthreebiediscount").html(
          sessionStorage.getItem("threebieSavings")
        );
      }
  ...
  $("#cardDetail")
    ...
    .html(
      "Redeemed Points " + $("#redee").find("#OrderSummaryShipping").text() + ""
    );
};
/* Added new function to fetch the data for Credit summary Start */
...
$("#CloseChangeAddress").on("click", function () {
  ...
  if ($("#CloseChangeAddress").hasClass("isreviewsectionvisible") == true) {
    ...
    $("#CloseChangeAddress").hasClass("ispaymentAccordianHeaderVisible") == true
  ) {
...
$("#closeMObAddresssection").on("click", function () {
  ...
  if (
    ...
    $("#closeMObAddresssection").hasClass("isreviewsectionvisible") == true
  ) {
    isAccordainopen("isreviewsectionvisible");
  } else if (
    $("#closeMObAddresssection").hasClass("ispaymentAccordianHeaderVisible") ==
    true
  ) {
    isAccordainopen("ispaymentAccordianHeaderVisible");
  }
...
$("#closeShiping,.closeShiping").click(() => {
  ...
  if ($("#useForBillingMenu").is(":checked")) {
    $("#shipFirstNameL").text(
      `${$("#hiddenShipFirstName").val()},${$("#hiddenShipLastName").val()}`
    );
    $("#ShipAddress1L").text(
      `${$("#hiddenShipAddress1").val()},${$("#hiddenShipAddress2").val()}`
    );
    $("#ShipZipL").text(
      `${$("#hiddenShipCity").val()},${$("#hiddenShipStateSelect").val()}, ${$(
        "#hiddenShipZip"
      ).val()}`
    );
    $("#BillFirstNameL").text(
    ...
    $("#BillZipL").text(
      `${$("#hiddenBillCity").val()},${$("#hiddenBillStateSelect").val()} ${$(
        "#hiddenBillZip"
      ).val()}`
    );
  } else {
...
function isAccordainopen(value) {
  if (
    value == "isdeliveryAccordianBodyvisible" &&
    value == "ispaymentAccordianHeaderVisible" &&
    value == "isreviewsectionvisible"
  ) {
    $("#deliveryAccordianBody").show();
    $("#paymentAccordianBody").hide();
    $("#revorder").show();
  ...
  if (value == "isreviewsectionvisible") $("#revorder").show();
...
function isDeliveryVisibleAll() {
  var isdeliveryAccordianBodyvisible = $("#deliveryAccordianBody").is(
    ":visible"
  );
...
function isPaymentVisible() {
  var ispaymentAccordianHeaderVisible = $("#paymentAccordianBody").is(
    ":visible"
  );
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (routegroup == getroutegroup) {
        ...
        $(".new-error-message")
          .html(
            `Zipcode entered is not valid for the route group assigned. Only FedEx Delivery method will be Available. <a onclick='$("#RouteGroupModal").modal()'>View Details</a>`
          )
          .show()
        ...
        $(".frtruckclass").remove();
        $("#m-willcall")
          .closest(".shippingCheckboxHeight")
          .css("display", "block");
      }
    },
  });
...
function getFreeShippingInfo() {
  ...
  var ShipEmail = encodeURIComponent(
    document.getElementById("hiddenShipEmail").value
  );
  ...
  jQuery.ajax({
    url:
      "/?subaction=fetch_shipping_details&Actions=ORDR&Screen=OUSL&Store_Code=G&ShipFirstName=" +
      shipFirstname +
      "&ShipLastName=" +
      ShipLastName +
      "&shipemail=" +
    ...
    success: function (data) {
      if (
        ...
        $(".showFreeShipping").html(
          "You've got <b>FREE HOME DELIVERY</b> and <b>Free Shipping</b> for this address"
        );
        shippingmessage = sessionStorage.setItem(
          "userrshippingmessage",
          "You've got <b>FREE HOME DELIVERY</b> and <b>Free Shipping</b> for this address"
        );
        $(".showimg")
          .html(
            '<img class="img-responsive" style="-webkit-transform: scaleX(-1);transform: scaleX(-1);float: right;" width="50" height="100" src="graphics/00000001/3/est.png"></img>'
          )
          .show();
      } else if (data.length == 1 && data[0].typeofshipping == "frp") {
        $(".fam-main").show();
        $(".showFreeShipping").html(
          "You've got <b>Free Shipping</b> for this address"
        );
        shippingmessage = sessionStorage.setItem(
          "userrshippingmessage",
          "You've got FLAT RATE SHIPPING for this address"
        );
        $(".showimg")
          .html(
            '<img class="img-responsive" style="-webkit-transform: scaleX(-1);transform: scaleX(-1);float: right;" width="50" height="100" src="graphics/00000001/3/est.png"></img>'
        ...
        shippingmessage = sessionStorage.setItem(
          "userrshippingmessage",
          "You've got FREE HOME DELIVERY for this address"
        );
        $(".showimg")
          .html(
            '<img class="img-responsive" style="-webkit-transform: scaleX(-1);transform: scaleX(-1);float: right;" width="50" height="100" src="graphics/00000001/3/est.png"></img>'
...
function checkCouponsApplied() {
  if (
    $('input[name="ShippingMethod"]').is(":checked") == false ||
    $("#selectedDeliveryContent").length < 1
  ) {
...
function GetCurbsySLots() {
  ...
  var url =
    "/Merchant5/merchant.mvc?Screen=mcurbsy&method=" +
    deliverymethod +
    "&customer_type=" +
    customerType +
    "&customer_id=" +
  ...
  $.get(url, function (dataval) {
    if (
      ...
      dataval.map((data) => {
        ...
        let Aslots = data.slots.filter(
          (item) => item.slot_group === "Afternoon"
        );
        ...
        newList.push({
          ...
          day: data?.day,
          slots: [
            { s_group: "Morning", slots: Mslots },
            { s_group: "Afternoon", slots: Aslots },
            { s_group: "Evening", slots: Eslots },
          ],
        });
      });
    ...
    curbsydata = new Vue({
      ...
      methods: {
        selectedSlots([
          ...
          var selectedDelivery =
            ...
            method +
            ' <span class="ft-w-6s mb-ft-12p ltr-sp-1s ltr-sp-0p5 ft-md">' +
            date_display +
            ' </span> </h5>  <h5 class="ft-w-6s mb-ft-12p ltr-sp-1s ltr-sp-0p5 ft-md">' +
            time_display +
            " <p></p></h5>";
          if (
            document.querySelector("input[name=curbsy_datanew]") !== null &&
          ...
          document.querySelector("#selectedDeliveryContent").innerHTML =
            selectedDelivery;
          $("input[name=formatted_delivery_date]").val(
            formatDeliveryDate(formatted_delivery_date)
          );
          $('input[name="delivery_date"]').val(
            selectedslot
          );
      ...
      mounted() {
        this.$nextTick(() => {
          ...
          document.querySelector(".shippingContentloaderbox").style.display =
            "none";
        });
      },
    });
    ...
    $(".submitcurbsy").on("click", function () {
      if (
        $(".dayslots.activeslot").find("span").attr("data-date_display") ==
        undefined
      ) {
        $("#loadcurbsydata")
          .find(".error-message")
          .html("Please select day slot")
          .show()
          .delay("5000")
          .fadeOut("fast");
      } else if (
        $(".timeslot.activeslot").find("span").attr("data-time_display") ==
        undefined
      ) {
        $("#loadcurbsydata")
          .find(".error-message")
          .html("Please select time slot")
          .show()
          .delay("5000")
          .fadeOut("fast");
      } else {
        var activeDay = 0;
        var date_display = $(".timeslot.activeslot")
          .find("span")
          .attr("data-date_display");
        var day = $(".timeslot.activeslot").find("span").attr("data-day");
        var isExpress = true;
        var method = $(".curbsyShippingRadio:checked").val().split(":").pop();
        var time_display = $(".timeslot.activeslot")
          .find("span")
          .attr("data-time_display");
        var ts = "";
        var slot = $(".timeslot.activeslot").find("span").attr("data-slot");
        var url = "";
        curbsydata.selectedSlots([
          activeDay,
          date_display,
          day,
          isExpress,
...
function GetDeliveryMethods() {
  ...
  var customerid = document.querySelector("#customerId").value;
  var shipFirstname = encodeURIComponent(
    document.getElementById("hiddenShipFirstName").value
  );
  var ShipLastName = encodeURIComponent(
    document.getElementById("hiddenShipLastName").value
  );
  var ShipEmail = encodeURIComponent(
    document.getElementById("hiddenShipEmail").value
  );
  var ShipPhone = encodeURIComponent(
    document.getElementById("hiddenShipPhone").value
  );
  var ShipAddress1 = encodeURIComponent(
    document.getElementById("hiddenShipAddress1").value
  );
  var ShipAddress2 = encodeURIComponent(
    document.getElementById("hiddenShipAddress2").value
  );
  var ShipCity = encodeURIComponent(
    document.getElementById("hiddenShipCity").value
  );
  var ShipStateSelect = encodeURIComponent(
    document.getElementById("hiddenShipStateSelect").value
  );
  var ShipCountry = encodeURIComponent(
    document.getElementById("hiddenShipCountry").value
  );
  var ShipZip = encodeURIComponent(
    document.getElementById("hiddenShipZip").value
  );
  var BillFirstName =
  ...
  var BillLastName =
    document.getElementById("hiddenBillLastName") === null
      ? ShipLastName
      : encodeURIComponent(document.getElementById("hiddenBillLastName").value);
  var BillPhone =
    document.getElementById("hiddenBillPhone") === null
      ? ShipPhone
      : encodeURIComponent(document.getElementById("hiddenBillPhone").value);
  var BillFax =
    document.getElementById("hiddenBillFax") === null
      ? ""
      : encodeURIComponent(document.getElementById("hiddenBillFax").value);
  var BillAddress1 =
    document.getElementById("hiddenBillAddress1") === null
      ? ShipAddress1
      : encodeURIComponent(document.getElementById("hiddenBillAddress1").value);
  var BillAddress2 =
    document.getElementById("hiddenBillAddress2") === null
      ? ShipAddress2
      : encodeURIComponent(document.getElementById("hiddenBillAddress2").value);
  var BillCity =
    document.getElementById("hiddenBillCity") === null
      ? ShipCity
      : encodeURIComponent(document.getElementById("hiddenBillCity").value);
  var BillStateSelect =
  ...
  var BillCountry =
    document.getElementById("hiddenBillCountry") === null
      ? ShipCountry
      : encodeURIComponent(document.getElementById("hiddenBillCountry").value);
  var BillZip =
    document.getElementById("hiddenBillZip") === null
      ? ShipZip
      : encodeURIComponent(document.getElementById("hiddenBillZip").value);
  var BillEmail =
    document.getElementById("hiddenBillEmail") === null
      ? ShipEmail
      : encodeURIComponent(document.getElementById("hiddenBillEmail").value);
  var BillToShow = document.getElementById("billing_to_show").value;
  var customertype =
    document.querySelector(".customertype") === null
      ? ""
      : document.querySelector(".customertype").value;

  var url =
    "/mcajax.html?mobileAction=ShipPay&customer_id=" +
    customerid +
    ...
    "&ShipPhone=" +
    ShipPhone +
    "&ShipFax=&ShipCompany=&ShipAddress1=" +
    ShipAddress1 +
    "&ShipResidential=0&ShipAddress2=" +
    ShipAddress2 +
    "&ShipCity=" +
    ShipCity +
    "&billing_to_show=" +
  ...
  $.get(url, function (dataval) {
    ...
    for (var i = 0; i < dataval.shipping_methods.length; i++) {
      ...
      if (dataval.shipping_methods[i].name.includes("FedEx International")) {
        $("body")
          .find("#INTERNATIONAL_ECONOMY-shipping")
          .attr(
            "data-estimated-date",
            FedEXmonthfull + " " + FedEXday + "," + " " + FedEXyear
          );
        $("body")
          .find("#INTERNATIONAL_PRIORITY-shipping")
          .attr(
            "data-estimated-date",
            FedEXmonthfull + " " + FedEXday + "," + " " + FedEXyear
          );
        dataval.shipping_methods[i].fedEXInternationdate =
        ...
        dataval.shipping_methods[i].fedEXInternalDateCheck =
          FedEXmonthfull + " " + FedEXday + "," + " " + FedEXyear;
      }
      shippingname = "";
      //  dataval.shipping_methods[i].shippingname = decodeHtmlCharCodes(dataval.shipping_methods[i].name);
      if (
        dataval.shipping_methods[i].code == "error" ||
        dataval.shipping_methods[i].code == "fallback" ||
        dataval.shipping_methods[i].code == ""
      ) {
        handleErrors = "error";
        $("#submitSelectedDelivery").hide();
      }
      dataval.shipping_methods[i].shippingname = dataval.shipping_methods[
        i
      ].name.replace("&reg;", "<sup>®</sup>");
    }
    loadCheckout = new Vue({
      el: "#shippingContainer",
      data: { loadshippingMethods: dataval },
      methods: {
        getEstimatedShipping() {
          let zip = $("#hiddenShipZip").val();
          let basketTotal = document.getElementById("basketTotal").value;
          let url =
            "/Merchant5/FedExApi/estimate-delivery-new.php?baskweight=" +
            basketTotal +
            "&destzip=" +
            zip;
          jQuery.ajax({
                  ...
                  jQuery.each(shippingEstimate, function (key, val) {
                    ...
                    if (key == "FEDEX_GROUND_ALT") {
                      ...
                      $("body")
                        ...
                        .attr("data-estimated-date", val);
                      $("body")
                        .find("#FEDEX_GROUND-shipping")
                        .attr("data-estimated-date", val);
                      $("body")
                        .find("#flatrate_shipping-shipping")
                        .attr("data-estimated-date", val);
                    } else {
                      $("body")
        ...
        loadPaymentMethods() {
          ...
          var PaymentMethod = $("#opay")
            .find('input[name="PaymentMethod"]')
            .val();
          var url =
            "/mcajaxweb.html?mobileAction=ShipPay&customer_id=" +
            customerid +
            "&Action=SHIP,PSHP,CTAX,update-order-custom-data&Store_Code=G&customer_type=" +
            customertype +
            "&pointspayment=true" +
            "&ShippingMethod=" +
            shippingMethod +
          ...
          $.get(url, function (dataval) {
            for (var i = 0; i < dataval.payment_methods.length; i++) {
              ...
              // TO get card id
              dataval.payment_methods[i].id = dataval.payment_methods[i].code
                .split(":")
                .pop();
            ...
            payment = new Vue({
              ...
              data: { loadpaymentmethods: dataval },
              mounted() {
                this.$nextTick(() => {
                  document.getElementById("loadpaymentmethods").style.display =
                    "block";
                  // changes added 19th Dec 2023
                  $(".newcard").show();
                  getCreditfeeSummary();
                  var DiscountApplied =
                    sessionStorage.getItem("DiscountApplied");
                  var lastStoredCard = sessionStorage.getItem("CardValue");
        ...
        loadRedeemPayment() {
          ...
          var url =
            "/mcajax.html?mobileAction=ShipPay&customer_id=" +
            customerid +
            "&Store_Code=G&customer_type=" +
            customertype +
            "&pointspayment=false&PaymentMethod=" +
            PaymentMethod;
          var image = "";
          ...
          $.get(url, function (dataval) {
            ...
            RedeemPayments = new Vue({
              ...
              data: { redeempayment: dataval },
              mounted() {
                this.$nextTick(() => {
                  document.getElementById(
                    "loadredeempaymentmethods"
                  ).style.display = "block";
                });
              },
            });
      ...
      mounted() {
        ...
        this.$nextTick(() => {
          ...
          $("#closeeDeliverty,.closeeDeliverty").click(() => {
            $("#applycoupon").show();
            var card = $("#cardDetail span").text();
            if ($("#cardDetail").text() == "Credit Card") {
            } else {
            ...
            if ($("#selectedDeliveryContent").children().length > 0) {
              // $("#continueDeliverymain").show();
            }
            ...
            if (
              $("#closeeDeliverty").hasClass("isreviewsectionvisible") == true
            ) {
              isAccordainopen("isreviewsectionvisible");
            } else if (
              $("#closeeDeliverty").hasClass(
                "ispaymentAccordianHeaderVisible"
              ) == true
            ) {
              isAccordainopen("ispaymentAccordianHeaderVisible");
              ...
              $("#closeMObAddresssection").hasClass(
                "isdeliveryAccordianBodyvisible"
              ) == true
            ) {
              isAccordainopen("isdeliveryAccordianBodyvisible");
            ...
            if (PaymentState == 0) {
              ...
              $(".continueOrder,.newcard").show();
              $(
                "#paymentAccordianBody,#paymentmethodForm,#reviewoderTabContent"
              ).show();
            }
            // Added on 18-12-2023
    ...
    $(".curbsyShippingRadio").on("click", function () {
      if ($(this).is(":checked") === true) {
        if (
          $(this).val().includes("Home Delivery") == true &&
          document.querySelector(".basktotal").innerHTML < 50
        ) {
    ...
    $("body").on("change", ".shippingMethodRadioNew", function () {
      let selectedShipingName = decodeHtmlCharCodes(
        $(this).attr("shipping-name")
      );
      var selectedDelivery =
        "<h5 class='ft-w-6s mb-ft-12p ltr-sp-1s ltr-sp-0p5 ft-md'>" +
        selectedShipingName +
        "</h5> <h5 class='estshiptime ft-w-6s mb-ft-12p ltr-sp-1s ltr-sp-0p5 ft-md'>" +
        $(this).attr("data-estimated-date") +
        "</h5>";
      $("#showfromattedDate").html(
        formatDeliveryDate($(this).attr("data-estimated-date"))
      );
      $('input[name="formatted_delivery_date"]').val(
        formatDeliveryDate($(this).attr("data-estimated-date"))
      );
    ...
    success: function (data) {
      if (data) {
        ...
        setTimeout(function () {
          ...
          $(
            "#reviewoderTabContent,#selectedDeliveryContent,#paymentAccordianBody"
          ).hide();
          $(".continueOrder,.newcard").hide();
...
$("#submitSavedAddress,#submitSavedAddressMobile").click(() => {
  ...
  $("#shippingAccordianBody").hide();
  $(".SavedShipping").show();
  $("#seletedHeaderContent").append(
    '<div style="position: absolute; width: 100%; height: 100%; background: transparent; z-index: 999; top: 0px; bottom: 0px; display: block;" class="selectedAddressloaderbox" hidden=""><div class="loader"></div></div>'
  );
  isLoaderVisible(".selectedAddressloaderbox");
  $(".new-error-message.error-message").hide().html("");
...
function getOtherDiscountinCheckout() {
  ...
  let url =
    "/checkout-ajax.html?CustomerAction=GetOtherSavings&displayType=raw";
  $.get(url, (response) => {
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      if (
        ...
        sessionStorage.setItem(
          "RewardPoints",
          "<span>Points " + usedpoints + "\n" + card + "</span>"
        );
        $(".usedpointss")
          .text("Points Used in Dollar  for this transaction $" + usedpoints)
          .show();
        $("#opay_form").show();
        $("#mivapay_frame").css("visibility", "visible");
        $("#cardDetail").html(
...
$(document).ready(function () {
  ...
  function checkHomedelivery() {
    $('input[name="ShippingMethod"]').click(function () {
      var orderSubtotalVal = parseFloat(
        $(".basket-total").text().replace("$", "")
      );
  ...
  $("body").on("change", "#billing_addressselect", function () {
    $("#hiddenBillStateSelect").val(
      $(this).find("option:selected").attr("data-state")
    );
    $("#hiddenBillFirstName").val(
      $(this).find("option:selected").attr("data-fname")
    );
    $("#hiddenBillLastName").val(
      $(this).find("option:selected").attr("data-lname")
    );
    $("#hiddenBillEmail").val(
      $(this).find("option:selected").attr("data-email")
    );
    $("#hiddenBillPhone").val(
      $(this).find("option:selected").attr("data-phone")
    );
    $("#hiddenBillFax").val($(this).find("option:selected").attr("data-fax"));
    $("#hiddenBillCompany").val(
      $(this).find("option:selected").attr("data-fname")
    );
    $("#hiddenBillAddress1").val(
      $(this).find("option:selected").attr("data-addr1")
    );
    $("#hiddenBillAddress2").val(
      $(this).find("option:selected").attr("data-addr2")
    );
    ...
    $("#hiddenBillCountry").val(
      $(this).find("option:selected").attr("data-cntry")
    );
  ...
  $(window).on("load", function () {
    ...
    $("select[name='ShipStateSelect']").css({ background: "none!important" });
    if ($("#ShipAddress1L").text().length > 1) {
      $(".shippingContentloaderbox").hide();
      if (isUserLoggedIn == 0) {
        getShippingMethods();
  ...
  $(".savedshippingAddress").change(function () {
    if ($("#useForBillingMenu").is(":checked")) {
      $(".savedshippingAddress").each(function () {
        if ($(this).is(":checked")) {
          $("#hiddenShipFirstName,#hiddenBillFirstName").val(
            $(this).attr("data-fname")
          );
          $("#hiddenShipLastName,#hiddenBillLastName").val(
            $(this).attr("data-lname")
          );
          $("#hiddenShipEmail,#hiddenBillEmail").val(
            $(this).attr("data-email")
          );
          $("#hiddenShipPhone,#hiddenBillPhone").val(
            $(this).attr("data-phone")
          );
          $("#hiddenShipFax,#hiddenBillFax").val($(this).attr("data-fax"));
          $("#hiddenShipCompany").val($(this).attr("data-fname"));
          $("#hiddenShipAddress1,#hiddenBillAddress1").val(
            $(this).attr("data-addr1")
          );
          $("#hiddenShipAddress2,#hiddenBillAddress2").val(
            $(this).attr("data-addr2")
          );
          $("#hiddenShipCity,#hiddenBillCity").val($(this).attr("data-city"));
          $("#hiddenShipState,#hiddenBillState").val(
            $(this).attr("data-state")
          );
          $("#hiddenShipZip,#hiddenBillZip").val($(this).attr("data-zip"));
          $("#hiddenShipCountry,#hiddenBillCountry").val(
            $(this).attr("data-cntry")
          );
        }
      });
    } else {
  ...
  $("#savedAddressSumbit,#savedAddressSumbitMobile").click(() => {
    ...
    $("#ShipZipL").text(
      `${$("#hiddenShipStateSelect").val()} ${$("#hiddenShipZip").val()}`
    );
  ...
  $("body").on("change", "#shipping_addressselect", function () {
    $("#hiddenShipStateSelect").val(
      $(this).find("option:selected").attr("data-state")
    );
    $("#hiddenShipFirstName").val(
      $(this).find("option:selected").attr("data-fname")
    );
    $("#hiddenShipLastName").val(
      $(this).find("option:selected").attr("data-lname")
    );
    $("#hiddenShipEmail").val(
      $(this).find("option:selected").attr("data-email")
    ...
    $("#hiddenShipCompany").val(
      $(this).find("option:selected").attr("data-fname")
    );
    $("#hiddenShipAddress1").val(
      $(this).find("option:selected").attr("data-addr1")
    );
    $("#hiddenShipAddress2").val(
      $(this).find("option:selected").attr("data-addr2")
    );
  ...
  $("body").on(
    "click",
    "#m-curbside,#m-home,#m-truckdelivery,#m-willcall,.checkRadio",
    function () {
      if ($("[name='ShippingMethod']:checked").val().indexOf("fedex") > 0) {
      } else {
        $("#fedexMethod").hide();
      }
  ...
  jQuery("body").on(
    "click",
    "#FEDEX_GROUND,#FEDEX_2_DAY,#STANDARD_OVERNIGHT,#m-FEDEX_GROUND,.shippingMethodRadio,.fallbackshippingMethodRadio",
    function (e) {
      ...
      if (
        $('#home:checked, :radio[name="rdFruit"]:checked').length ||
        $('#curbcall:checked, :radio[name="rdFruit"]:checked').length ||
        $('#willcallRadio:checked, :radio[name="rdFruit"]:checked').length ||
        $('#truckdelivery:checked, :radio[name="rdFruit"]:checked').length
      ) {
  ...
  $("#continueDelivery,#continueDeliverymain").click(() => {
    ...
    if ($("#curbsy-div").is(":visible") == true) {
      ...
      if (iframe.contents().find(".days").hasClass("active") == false) {
        iframe
          .contents()
          .find("#curbsy-app .error-message")
          .text("Please Select Day Slots")
          .show();
      } else if (
        iframe.contents().find("#curbsy-app .error-message").is(":visible") &&
      ...
      $("#selectedDeliveryContent").html(
        $("#selectedDeliveryContentforFedExShipping").html()
      );
      $(".applyDisc").show();
      $("#opay").submit();
...
var changeBasketQty = function (element, sign) {
  ...
  timer_id = setTimeout(function () {
    ...
    $(element)
      .closest(".custom-number-input")
      .find(".bask-item-quantity-update-button")
      .click();
  }, 500);
};
  ...
  $(window).on("load", function () {
    ...
    if (allowCheckout > 0) {
      setTimeout(function () {
        ...
        console.log("excuted after 500 of page load"), 500;
      });
    }
...
function checkFocus() {
  if (document.activeElement == document.getElementById("curbsy")) {
    ...
    if (
      $("[name='ShippingMethod']:checked").val() ===
      "flatrate:FR Truck Delivery"
    ) {
      /* for fr truck */
...
$("#curbsy").on("load", function () {
  ...
  if (
    $("[name='ShippingMethod']:checked").val() === "flatrate:FR Truck Delivery"
  ) {
    /* for fr truck */
    var frtruckcounter = 0;
    iframe.find(".childcard").each(function () {
...
jQuery(document).ready(function () {
  ...
  window.openCurbsy = function (method) {
    ...
    $("body").on(
      "click",
      "#m-curbside,#m-willcall,#m-home,#m-truckdelivery",
      function (e) {
    ...
    setTimeout(function () {
      ...
      $("#curbsy-div .fa").css(
        "left",
        $("#curbsy-wrapper").position().left + $("#curbsy-wrapper").width() - 11
      );
      $("#curbsy-div .fa").css("top", $("#curbsy-wrapper").position().top - 15);
    }, 2000);
  };
  ...
  window.closeCurbsy = function () {
    ...
    var last = localStorage.curbsyLastSelect
      ? JSON.parse(localStorage.curbsyLastSelect)
      : {};
  ...
  setInterval(function () {
    ...
    if (selectedSM != undefined) {
      ...
      if (
        last &&
        (selectedSM == "flatrate:Curbside Pickup" ||
          selectedSM == "flatrate:Curbside Pickup Express" ||
          selectedSM == "flatrate:Home Delivery Express" ||
          selectedSM == "flatrate:Home Delivery")
      ) {
        ...
        formatteddelivetdate =
          ("0" + (formatteddelivetdate.getMonth() + 1)).slice(-2) +
          "/" +
          ("0" + formatteddelivetdate.getDate()).slice(-2) +
          "/" +
          formatteddelivetdate.getFullYear();
...
$(
  ".new_addresss,#addnewAddress,#addnewAddresss,#addnewaddress,#changeBillingaddress,#changeBillingaddressMobile"
).on("click", function () {
  $("#shippingAddress").hide();
  $("#shippingAccordianBody").hide();
...
function getThreebieDiscountinCheckout() {
  basketApp.loadBasket();
  let url =
    "/checkout-ajax.html?CustomerAction=GetThreebiefromBask&displayType=raw";
  $.get(url, (response) => {
...
var onclick_submit1 = function (event) {
  ...
  if (checkout_store_data == 1) {
    ...
    var showfromattedDate = $("#showfromattedDate").html();
    if (TDate(showfromattedDate) == false) {
    } else {
...
function CalculatePointsThreshold() {
  ...
  if (parseFloat(points) >= parseFloat(basketTotal)) {
    ...
    $(".showerrormessage")
      .text("Max Redeemable amount " + newtotalamount)
      .show();
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      ...
      setTimeout(function () {
        ...
        var newremdeem =
          parseFloat(newbasketTotal) - (1 / 100) * parseFloat(newbasketTotal);
...
function ApplyCoupon(elemet) {
  var coupon = elemet.target.innerHTML
    .replaceAll("<h1>", "")
    .replaceAll("</h1>");
  $(".discountcodeinput").val(coupon);
  $("#applyCouponForm").on("submit", function (e) {
...
const getRedeemedOrderSummary = () => {
  let url =
    "/ajax.html?CustomerAction=getbasketcharges&displayType=raw&redeemed=true";
  $.get(url, (response) => {
    $("#showbasketCharges").html(response);
    if (screen.width < 768) {
      $("#mobileOrderSummary").html(response);
...
$(document).ready(function () {
  if (document.getElementById("hiddenShipZip").value == "") {
    $(".addnewAddress").modal({ backdrop: "static", keyboard: false }, "show");
  }
});
...
function updateDetailsTextWithJquery() {
  ...
  const url = "/?Screen=checkout-ajax&customerAction=checkoutmessages&ship_zip="+zipcode+'&specialshipping='+specialshipping+"&ship_state="+ship_state+"&iswinepresent="+iswinepresent;
