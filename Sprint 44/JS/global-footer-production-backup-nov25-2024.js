...
document.addEventListener("DOMContentLoaded", () => {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function adjustContainerHeight() {
      ...
      container.style.height = `${screenHeight * 0.8}px`; // Adjust the 0.8 to desired ratio
    }
...
var OrderguideSearchProducts = new Vue({
  ...
  methods: {
    ...
    makeAjaxRequest(url, jsonRequest) {
...
function AddItemstoWishlist(buyBotton) {
  ...
  if (type === "deleteWisshlistItem") {
    ...
    $.ajax({
      ...
      success: function (t) {
        $('#app-wish').html('');
        $(".wishlistitems_container").load(
          window.location.protocol +
          "//" +
          window.location.host +
          window.location.pathname +
          "?Screen=WISH&WishList_ID=" + wishlist_id + "&Per_Page=" + per_page + "&offset" + offset_value + "&sort_by=" + sort_by + " #app-wish",
          function () {
