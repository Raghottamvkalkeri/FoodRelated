...
$(document).on("click.outsideAccountWrapper", function (e) {
    if (ignoreNextClick) {
...
$(
    ".createAccount,.businessaccount,#createAccountBtnBusiness,.createAccountBtnBusiness"
).click(function () {
    $.getScript("scripts/00000001/google-address.js");
});
...
$(document).ready(function () {
    $(".updatetocartbtn").on("tap mousedown", function (event) {
...
function GetFreeshippingInfo() {
    ...
    if (shipzip != "") {
        jQuery.ajax({
            url: "/?subaction=fetch_shipping_details&Action=ORDR&Screen=OUSL&Store_Code=G&ShipFirstName=" +
                shipfirstname +
                "&ShipLastName=" +
                shiplastname +
                "&shipemail=" +
        ...
        if (data.groups.length) {
            $.each(data.groups, function (i, item) {
                ...
                if (data_stock > 0) {
                    ...
                    $(".checkpcode-" + popupid)
                        .find(".data-min")
                        .attr("data-incart", item.quantity);
                    $(".ProductDetail-" + popupid)
                        .find('input[value="' + item.code + '"')
                        .attr("data-incart", item.quantity);
                    $(".ProductDetail-" + popupid)
                        .find('input[value="' + item.code + '"')
                        .attr("data-added", item.quantity);
                    $(".ProductDetail-" + popupid)
                        .find('input[value="' + item.code + '"')
                        .attr("data-basket-group", item.group_id);
                    ...
                    $(".ProductDetail-" + popupid)
                        .find(".out-of-stock-new,.soldoutmessage")
                        .css("display", "none");
                    if (
                        $(radiocheck).attr("data-added") &&
                        $(radiocheck).is(":checked")
                    ) {
                        $(".ProductDetail-" + popupid)
                        ...
                        function buttonInput(element) {
                            /*var prevQuantity = $(".ProductDetail-" + popupid).find('input[name="Quantity"]').val();
                        ...
                        $(".ProductDetail-" + productcode + ":visible")
                            .find(".updatetocartbtn")
                            .bind("keyup", function () {
...
document.addEventListener("click", function (event) {
    ...
    if (
        ...
        !document.querySelector(".mm_searchfield").contains(event.target)
    ) {
        searchContainer.style.display = "none";
                        ...
                        methods: {
                                                ...
                                                success: function (retryResult) {
                                                    ...
                                                    if (retryResult.data.length > 0) {
                                                        document.querySelector(
                                                            ".ShoppingRecentRecommendation"
                                                        ).innerHTML = "Trending Search....";
                                                        $("#searchHistory")
                            ...
                            getTrendingHistory: function () {
                                ...
                                $.ajax({
                                    ...
                                    dataType: "json",
                                    success: function (result) {
                                        self.trendingHistory = self.filterTrendingHistory(
                                            result.data
                                        );
                            ...
                            filterTrendingHistory: function (trending) {
                                ...
                                return trending.filter(function (item) {
                                    var isExcluded = self.searchHistory.some(function (
                                        historyItem
                                    ) {
                                        return (
                                            historyItem.search_keyword.toLowerCase() ===
                                            item.search_keyword.toLowerCase()
                                        );
                                    });
                                    return !isExcluded;
                                });
                                    ...
                                    mounted: function () {
                                        this.$nextTick(function () {
                                            ...
                                            if (isUserLoggedIn === 1 && result.length) {
                                                $("#searchTrendingHistory")
                                                    .find(".material-symbols-outlined")
                                                    .text("trending_up");
                                                document.querySelector(
                                                    ".ShoppingTrendingRecommendation"
                                                ).style.display = "block";
                                            } else {
                                                $("#searchTrendingHistory")
                                                    .find(".material-symbols-outlined")
                        ...
                        mounted: function () {
                            this.$nextTick(function () {
                                ...
                                if (this.products.length > 0) {
                                    ...
                                    document.querySelector(".s-seperator").style.display =
                                        "block";
                                } else {
                                    document.querySelector(
                                        ".ShoppingRecommendation"
...
var getFlatRateData = new Vue({
    ...
    methods: {
        fetchShippingOptions: function () {
            ...
            var zip = document.querySelector(
                ' #shippingpromoroform input[name="ShipEstimate:ship_zip"]'
            ).value; // Get the entered zip code

            // Update the URL with the entered zip code
            var url =
                "/Merchant5/merchant.mvc?Screen=CUDET&mobileAction=getFlatRate&zipcode=" +
                zip;
    ...
    mounted: function () {
        ...
        document
            .querySelector("#shippingpromoroform")
        ...
        document
            .querySelector('#shippingpromoroform input[name="ShipEstimate:ship_zip"]')
...
let splitwineorder = new Vue({
    ...
    methods: {
        ...
        fetchNonFedExItems() {
            $.ajax({
                url: "/cudet.html?CustomerAction=nonfedexproducts",
                method: "GET",
                success: (response) => {
                    // Process and display the result
                    this.nonfedexitems = response.items.filter(data => data.nofedex == 1);
                    console.log(this.nonfedexitems);
                },
                error: (error) => {
                    console.error("Error fetching non fedex items", error);
                },
            });
        },
...
let nofedexitem = new Vue({
    el: "#removeItemsfromBask",
    data: {
        nonfedexitems: [],
        nonfedexitemlist: [],
    },
    methods: {
        fetchNonFedExItems() {
            $.ajax({
                ...
                success: (response) => {
                    // Process and display the result
                    this.nonfedexitemlist = response;
                    this.nonfedexitems = response.items.filter(data => data.nofedex == 1);
                    $('#removeItemsfromBask').modal();
                    $('.nofedexitemlist').show();
        ...
        removeNonFedExItemsfromBasket() {
            let line_id = document.querySelector('#nonfedexitemlist').value;
            const basketGroups = line_id.replace(/[\[\]']/g, "").split(","); // Replace with actual Basket_Group IDs

            async function deleteBasketGroup(basketGroup) {
                const url = `/?Action=RGRP&Basket_Group=${basketGroup}`;

                try {
                    const response = await fetch(url, {
                        method: 'GET'
                    }); // Replace with the correct HTTP method if needed
                    if (!response.ok) {
                        throw new Error(`HTTP error! Status: ${response.status}`);
                    }
                    console.log(`Successfully deleted Basket_Group ${basketGroup}`);

                } catch (error) {
                    console.error(`Error deleting Basket_Group ${basketGroup}:`, error);
                }
            }

            async function deleteAllBasketGroups() {
                const deletePromises = basketGroups.map(group => deleteBasketGroup(group));
                await Promise.all(deletePromises); // Wait for all delete operations to complete
                window.location.reload(); // Reload the page after all deletions are done
            }

            // Call the function
            deleteAllBasketGroups();
        }
    },
    mounted() {
        document.body.addEventListener("click", (event) => {
            if (event.target && event.target.id === "removefromBasket") {
                if (typeof nofedexitem.fetchNonFedExItems === "function") {
                    nofedexitem.fetchNonFedExItems();
                } else {
                    console.error("nofedexitem.fetchNonFedExItems is not defined.");
                }
            }
        });
    },
...
let multilogin = new Vue({
    ...
    methods: {
        multiaccountlist() {
            ...
            $.ajax({
                url: "/majax.html?mobileAction=getUniversalAccount&customer_id=" + customer_id,
                method: "GET",
                success: (response) => {
                    // Process and display the result
                    this.multiaccountlists = response;
                    console.log(this.multiaccountlists);
                },
                error: (error) => {
                    console.error("Error fetching non fedex items", error);
                },
...
var OrderguideSearchProducts = new Vue({
    ...
    methods: {
        ...
        makeAjaxRequest(url, jsonRequest, showLoader = true, toggleVisibility = true, source) {
            ...
            $.ajax({
                ...
                success(result) {
                    if (source !== 'getWishlist') {
                        // console.log(result);
                        self.processProducts(result);
                    } else {
                        console.log(self.getWishlistName(result));
                    }
        ...
        refreshDataOnCheckboxChange() {
            ...
            this.makeAjaxRequest(`/?Screen=SRCHJSON&Search=${searchKey}&SearchOffset=0&Sort_By=${sort_by}&per_page=10&recipes=No&&x=0&y=0&`, jsonRequest, false, false); // No loader
        },
        getWishlist() {
            var sessionid = document.getElementById("sessionid").value;
            var customerType = document.querySelector(".customertype").value;
            const jsonRequest = '{"Store_Code": "G","Function":"Module","Session_Type":"runtime","Module_Code":"frjsonfunctions","Module_Function":"FR_GetWishLists","customer_session":"' +
                sessionid +
                '","customer_type":"' +
                customerType +
                '"}';
            this.makeAjaxRequest(`/Merchant5/json.mvc`, jsonRequest, true, true, 'getWishlist');
        }
    },
    ...
    mounted() {
        ...
        const self = this;
        document
            .querySelector("#OrdguidesearchProductTxt")
        ...
        document
            .querySelector("#ordguideproducts") // Replace with the parent container or a common ancestor
        ...
        document
            .querySelector(".close1") // Replace with the parent container or a common ancestor
            .addEventListener("click", function (event) {
...
function AddItemstoWishlist(buyBotton) {
    ...
    var wishlistid = $("input[name=WishList_ID]")
        .val();
